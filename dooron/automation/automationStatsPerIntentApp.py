...
from constants.helpers import get_intents_and_fill_na_dooron
from constants.importantDates import last_week, last_month, last_six_months
from constants.mongoConnectDooron import mutationLogger_collection, app_webhooks, fb_users
...
def filter_admins(app_webhook_df):
    ...
    admin_users_list = list(fb_users.find({
...
def main_func():
    app_df = get_app_webhooks(250000)
    ...
    print(f'Num of total auto messages not initiated by us:\n'
          ...
          f'{sent_by_us_df["isAuto"].value_counts(normalize=True)}'
          f'\nfor {sent_by_us_df["to"].nunique()} users')
    sent_by_us_df['intentsBeforeCat'] = sent_by_us_df.apply(lambda row: get_intents_and_fill_na_dooron(row), axis=1)
